...
import {BlockHeaderMerkleParts} from "./libraries/BlockHeaderMerkleParts.sol";
import {MultiStore} from "./libraries/MultiStore.sol";
import {SafeMath} from "@openzeppelin/contracts/math/SafeMath.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {IAVLMerklePath} from "./libraries/IAVLMerklePath.sol";
import {TMSignature} from "./libraries/TMSignature.sol";
import {Utils} from "./libraries/Utils.sol";
import {Packets} from "./libraries/Packets.sol";
...
contract Bridge is IBridge, Ownable {
    ...
    /// Initializes an oracle bridge to Odin.
    /// @param _validators The initial set of Odin active validators.
    ...
    /// @param _validators The changed set of Odin validators.
    function updateValidatorPowers(ValidatorWithPower[] memory _validators) external onlyOwner {
        for (uint256 idx = 0; idx < _validators.length; ++idx) {
            ValidatorWithPower memory validator = _validators[idx];
    ...
    ) external override returns (RequestPacket memory, ResponsePacket memory) {
        (bytes memory relayData, bytes memory verifyData) = abi.decode(_data, (bytes, bytes));
        (bool relayOk,) = address(this).call(abi.encodePacked(this.relayOracleState.selector, relayData));
    ...
    function relayAndMultiVerify(
        ...
        (bytes memory relayData, bytes[] memory manyVerifyData) = abi.decode(_data, (bytes, bytes[]));
        (bool relayOk,) = address(this).call(abi.encodePacked(this.relayOracleState.selector, relayData));
        require(relayOk, "RELAY_ORACLE_STATE_FAILED");

        RequestPacket[] memory requests = new RequestPacket[](manyVerifyData.length);
        ResponsePacket[] memory responses = new ResponsePacket[](manyVerifyData.length);
        ...
        for (uint256 i = 0; i < manyVerifyData.length; i++) {
            ...
            require(verifyOk, "VERIFY_ORACLE_DATA_FAILED");
            (requests[i], responses[i]) = abi.decode(verifyResult, (RequestPacket, ResponsePacket));
        }
    ...
    /// Verifies that the given data is a valid data on Odin as of the given block height.
    ...
    function verifyOracleData(
        ...
        vars.dataHash = sha256(Packets.getEncodedResult(_requestPacket, _responsePacket));
