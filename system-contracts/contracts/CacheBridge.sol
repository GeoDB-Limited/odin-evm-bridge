...
import {BridgeUtils} from "./libraries/BridgeUtils.sol";
import {IBridge} from "./interfaces/IBridge.sol";

/// @title CacheBridge Odin
contract CacheBridge is Bridge, ICacheBridge {
    ...
    /// Initializes an oracle bridge to Odin by pass the argument to the parent contract (Bridge.sol).
    /// @param _validators The initial set of Odin active validators.
    constructor(ValidatorWithPower[] memory _validators) Bridge(_validators) {}
    ...
    function getLatestResponse(RequestPacket memory _request) public override view returns (ResponsePacket memory) {
    ...
    function relay(bytes calldata _data) external override {
        (RequestPacket memory request, ResponsePacket memory response) = this.relayAndVerify(_data);
    ...
    function relayMulti(bytes calldata _data) external override {
        (RequestPacket[] memory requests, ResponsePacket[] memory responses) = this.relayAndMultiVerify(_data);
    ...
    function cacheResponse(RequestPacket memory _request, ResponsePacket memory _response) internal {
